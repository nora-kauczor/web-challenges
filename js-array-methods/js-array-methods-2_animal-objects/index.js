...
const firstAnimalStartingWithLetterG = animals.find((animal) => {
  return animal.name.toLowerCase().startsWith("g");
});
...
//   animals.filter((animal) => animal.continents.includes("Europe"));
...
// const withoutObjects = africanAnimals.forEach(
//   (animal) => (animal.weight = weight)
// );
// console.log(withoutObjects);
...
const averageWeightOfAllAnimalsInAfrica =
  ...
  animals
    .filter((animal) => animal.continents.includes("Africa"))
    .map((animal) => animal.weight)(animals.length + 1);
