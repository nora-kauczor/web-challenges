...
function findByLastName(arrayParameter, lastNameParameter) {
  return arrayParameter.find(
    (employee) => employee.lastName === lastNameParameter
  );
  // Exercise 2:
  // The some as above, but now you are searching by the 'id'
}

console.log(findByLastName(employees, "Schilder"));

function findById(employees, id) {
  return employees.find((employee) => employee.id === id);
}
console.log(findById(employees, "0.5252339382028444"));
// Exercise 3:
...
function findByCitySubString(employees, string) {
  return employees.find((employee) => employee.city.startsWith(string));
}

console.log(findByCitySubString(employees, "K"));
...
function findByProfessionSubStringAndAge(employees, subString, age) {
  return employees.find(
    (employee) => employee.profession.includes(subString) && employee.age > age
  );
}
console.log(findByProfessionSubStringAndAge(employees, "demic", 35));
