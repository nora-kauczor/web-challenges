...
colorSlider.addEventListener("click", () => {
  ...
  box.style.backgroundColor = `hsl(${colorPosition}, 70%, 60%)`;
});

radiusSlider.addEventListener("click", () => {
  const radiusPosition = radiusSlider.value;
  box.style.borderRadius = `${radiusPosition}%`;
});

rotationSlider.addEventListener("click", () => {
  const rotationPosition = rotationSlider.value;
  box.style.transform = `rotate(${rotationPosition}deg)`;
});
